...
class FilterViewController: UIViewController {
    ...
    internal func apply() {
        ...
        guard let filterType = filterType, let filter = imageView.image?.filter(with: filterType, intensity: filterIntensity) else {
            return
        }
        imageView.image?.apply(filter, completion: { (image) in
