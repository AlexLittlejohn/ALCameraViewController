...
open class CameraViewController: UIViewController {
     ...
     */
    fileprivate func addRotateObserver() {
        NotificationCenter.default.addObserver(
            self,
     ...
     */
    fileprivate func setupVolumeControl() {
     ...
     * state of the camera.
     */
    fileprivate func toggleButtons(_ enabled: Bool) {
        [cameraButton,
            closeButton,
            swapButton,
    ...
    internal func rotate(_ actualInterfaceOrientation: UIInterfaceOrientation) {
         ...
         */
        let time = DispatchTime(uptimeNanoseconds: DispatchTime.now().uptimeNanoseconds) + Double(1 * Int64(NSEC_PER_SEC)/10) / Double(NSEC_PER_SEC)
     ...
     */
    fileprivate func showNoPermissionsView(_ library: Bool = false) {
